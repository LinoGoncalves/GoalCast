...
class AgenticSDLCCLI:
    ...
    def list_agents(self) -> None:
        ...
        for agent_name in sorted(agents):
            ...
            if agent.capabilities:
                print(
                    f"   Capabilities: {', '.join(agent.capabilities[:3])}{'...' if len(agent.capabilities) > 3 else ''}"
                )
            print()
    ...
    def show_status(self, project_id: str) -> None:
        ...
        try:
            ...
            project_tasks = [
                task
                for task in self.sdlc.workflow_engine.active_tasks.values()
        ...
        while True:
            try:
                ...
                if cmd == "exit":
                    ...
                    print(
                        "Available commands: list-agents, create-project, start-workflow, status, execute-task, help, exit"
                    )
